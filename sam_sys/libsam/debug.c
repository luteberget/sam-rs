...
phonemesTestCallback phonemesTestCallbackFn = NULL;
framesTestCallback framesTestCallbackFn = NULL;

void SetPhonemesTestCallback(phonemesTestCallback f) {
    phonemesTestCallbackFn = f;
}

void SetFramesTestCallback(framesTestCallback f) {
    framesTestCallbackFn = f;
}


void PrintPhonemes(int call_site, unsigned char *phonemeindex, unsigned char *phonemeLength, unsigned char *stress)
{
    if(phonemesTestCallbackFn != NULL) {
        phonemesTestCallbackFn(call_site, phonemeindex, phonemeLength, stress);
    }
...
void PrintOutput(int call_site,
    ...
    unsigned char *p)
{
    if(framesTestCallbackFn != NULL) {
        framesTestCallbackFn(call_site, flag, f1, f2, f3, a1, a2, a3, p);
    }
